...
int main()
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      // ARP frame nes source Hardware type, length, protocol type and IP length
      // IP/MAC and target IP/MAC
      printf("ARP Type\n");
      struct arphdr arp_h;

      arp_h.ar_hrd = 1; /* Format of hardware address.  unsigned short int*/
      arp_h.ar_hln = '6';          /* Length of hardware address.  unsigned char*/
