...
int main(){
  int packet_socket;
  //get list of interface addresses. This is a linked list. Next pointer is in ifa_next, interface name is in ifa_name, address is in ifa_addr. You will have multiple entries in the list with the same name, if the same interface has multiple addresses. This is common since most interfaces will have a MAC, IPv4, and IPv6 address. You can use the names to match up which IPv4 address goes with which MAC address.
  ...
  for(tmp = ifaddr; tmp!=NULL; tmp=tmp->ifa_next){
    //Check if this is a packet address, there will be one per interface.  There are IPv4 and IPv6 as well, but we don't care about those for the purpose of enumerating interfaces. We can use the AF_INET addresses in this list for example to get a list of our own IP addresses
  ...
  //loop and recieve packets. We are only looking at one interface, for the project you will probably want to look at more (to do so, a good way is to have one socket per interface and use select to see which ones have data)
  ...
  while(1){
    ...
    //ignore outgoing packets (we can't disable some from being sent by the OS automatically, for example ICMP port unreachable messages, so we will just ignore them here)
